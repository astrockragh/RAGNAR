...
- `resolution = $\infty$`: The resolution of the spectrograph, as defined in Appendix \S \ref{app_sec:resolution`. The default is $\infty$, since infinite resolution added with a non-zero `line\_spread`, will not change the `line\_spread`. If the resolution is not $\infty$ or `None`, `line\_spread` will be automatically set to 0.
...
- `Trots = [175, 190, 205]`: Array or list of the rotational temperatures [K]. This is the $T_{cold`$ described in \S \ref{subsec:OH`.
...
- `importance\_limit\_group = 2e-2`: If a subline contributes less than this fraction of the total intensity at line centre, do not include it in the label. However, all lines that contribute down to `importance\_limit\_group$^3$` are included in `originIds` column (see \S \ref{app_sec:outputs`). This makes precision analysis of each line possible if needed.
- `chem\_scatters = 3`: The number of samples from the species concentration distribution to take, according to the approach laid out in \S \ref{sec:simulation`.
...
- `aerosols = ['background', 'volcanic']`: List of aerosol models to loop over for the transmission curves. 'background', and 'volcanic' are possible options. See Figure \ref{fig:transmission` to see the behaviour of these two models.
...
- `important\_species = ['combin', 'aercld', 'molec', 'H2O', 'O3', 'O2', 'CO2', 'CH4']`: Possible absorption origins to consider for determining which species cause absorption. See below in \S \ref{app_sec:outputs` for a more detailed description of each species.
